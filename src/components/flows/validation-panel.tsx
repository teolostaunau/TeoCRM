  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        {errors.length}{" "}
        {errors.length === 1
          ? t("flows.validation.errors.one")
          : t("flows.validation.errors.other")}
        ,{" "}
        {warnings.length}{" "}
        {warnings.length === 1
          ? t("flows.validation.warnings.one")
          : t("flows.validation.warnings.other")}
      </div>
  ...
  if (issue.node_key && onJump) {
    return (
      <button
        ...
        aria-label={t("flows.validation.jumpToNode", { nodeKey: issue.node_key })}
      >
        {body}
      </button>
    );
