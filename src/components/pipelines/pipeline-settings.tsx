...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    if (renameRes.error || stagesRes.error) {
      toast.error(t("pipelines.settings.validation.saveError"));
      return;
    }
    ...
    onPipelinesChanged();
    onStagesChanged();
    toast.success(t("pipelines.settings.validation.saveSuccess"));
  }
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error(t("pipelines.settings.validation.addStageError"));
      return;
    }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("pipelines.settings.validation.removeStageError"));
      return;
    }
    const { error } = await supabase
      ...
      .eq("id", stageId);
    if (error) {
      toast.error(t("pipelines.settings.validation.deleteStageError"));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("pipelines.settings.validation.deletePipelineError"));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
    toast.success(t("pipelines.settings.validation.deletePipelineSuccess"));
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-popover border-border max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t("pipelines.settings.dialogTitle")}</DialogTitle>
        </DialogHeader>
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-red-400">
                  {t("pipelines.settings.deleteConfirmTitle")}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("pipelines.settings.deleteConfirmDescription")}
                </p>
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? t("pipelines.settings.actions.deleting") : t("pipelines.settings.actions.deletePipeline")}
              </Button>
            </div>
          </div>
        ) : (
          <>
            <div className="grid gap-4 py-2">
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("pipelines.settings.labels.pipelineName")}</Label>
                <Input
                  value={name}
              ...
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("pipelines.settings.labels.stages")}</Label>
                <DndContext
                  sensors={sensors}
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {STAGE_COLORS.map((color) => (
                    <button
                      ...
                      aria-label={t("pipelines.settings.placeholders.colorAria", { color })}
                    />
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    placeholder={t("pipelines.settings.placeholders.newStageName")}
                  ...
                  >
                    ...
                    {t("pipelines.settings.actions.add")}
                  </Button>
                </div>
              </div>
              ...
              >
                <Plus className="mr-1 h-3 w-3" />
                {t("pipelines.settings.actions.createNewPipeline")}
              </Button>
            </div>
            ...
            <DialogFooter className="border-border bg-popover/50">
              ...
              >
                {saving ? t("pipelines.settings.actions.deleting") : t("pipelines.settings.actions.saveChanges")}
              </Button>
            </DialogFooter>
          </>
        )}
  ...
  return (
    ...
    >
      <button
        ...
        aria-label={t("pipelines.settings.placeholders.dragAria")}
      >
