...
export function PasswordForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (next.length < MIN_PASSWORD) {
      setConfirmError(t("settings.password.errors.tooShort", { min: MIN_PASSWORD }));
      return;
    }
    if (next !== confirm) {
    ...
    try {
      ...
      if (signInError) {
        toast.error(t("settings.password.errors.incorrect"));
        return;
      }
      ...
      if (updateError) {
        toast.error(t("settings.password.errors.updateFailed", { error: updateError.message }));
        return;
      }
