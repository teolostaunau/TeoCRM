...
export function ProfileForm() {
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_MIME.has(file.type)) {
      toast.error(t("settings.profile.avatar.unsupportedType"), {
        description: t("settings.profile.avatar.unsupportedTypeDesc"),
      });
      return;
    }
    if (file.size > MAX_AVATAR_BYTES) {
      toast.error(t("settings.profile.avatar.tooLarge"), {
        description: t("settings.profile.avatar.tooLargeDesc"),
      });
      return;
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedName) {
      toast.error(t("settings.profile.displayName.required"));
      return;
    }
    const trimmedEmail = email.trim();
    if (!EMAIL_RE.test(trimmedEmail)) {
      toast.error(t("settings.profile.email.invalid"));
      return;
    }
    ...
    try {
      ...
      if (trimmedEmail.toLowerCase() !== profile.email.toLowerCase()) {
        ...
        if (emailError) {
          ...
          toast.success(t("settings.profile.email.saved"));
          toast.error(t("settings.profile.email.changeFailed", { error: emailError.message }));
          setSaving(false);
      ...
      toast.success(
        emailSent
          ? t("settings.profile.email.savedConfirm")
          : t("settings.profile.email.saved"),
      );
    } catch (err) {
          ...
          <div className="flex flex-wrap items-center gap-5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                ...
                {currentAvatar ? t("settings.profile.avatar.changePhoto") : t("settings.profile.avatar.uploadPhoto")}
              </Button>
              {currentAvatar && (
                <Button
                  type="button"
              ...
              <p className="w-full text-xs text-muted-foreground">
                {t("settings.profile.avatar.helpText")}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {emailChangePending && (
              <p className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
                ...
                <span dangerouslySetInnerHTML={{ __html: t("settings.profile.email.pendingWarning", { oldEmail: profile?.email ?? "", newEmail: email }) }} />
              </p>
            )}
          </div>
