...
export function SessionsCard() {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      if (error) {
        toast.error(t('settings.sessions.errors.signOutFailed', { error: error.message }));
        return;
      }
      window.location.href = '/login';
    } catch (err) {
      const msg = err instanceof Error ? err.message : t('settings.common.unknownError');
      toast.error(msg);
    } finally {
      setSigningOut(false);
    }
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={onConfirm} disabled={signingOut}>
              {signingOut ? (
                ...
                t('settings.sessions.signOutButton')
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
