...
export function ContactForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!phone.trim()) {
      toast.error(t('contacts.form.validation.phoneRequired'));
      return;
    }
    ...
    if (!isEdit && dupMatch?.exact) {
      toast.error(t('contacts.form.validation.phoneExists'));
      return;
    }
    ...
    try {
      ...
      if (!user) throw new Error(t('contacts.form.validation.authRequired'));
      if (!accountId) throw new Error(t('contacts.form.validation.accountRequired'));
      ...
      toast.success(
        isEdit
          ? t('contacts.form.validation.saveSuccessEdit')
          : t('contacts.form.validation.saveSuccess'),
      );
      onOpenChange(false);
      onSaved();
    } catch (err: unknown) {
      ...
      const message = err instanceof Error ? err.message : t('contacts.form.validation.saveError');
      toast.error(message);
    } finally {
      setSaving(false);
    }
      ...
      <DialogContent className="bg-popover border-border text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">
            {isEdit ? t('contacts.form.title.edit') : t('contacts.form.title.add')}
          </DialogTitle>
          <DialogDescription className="text-muted-foreground">
            {isEdit
              ? t('contacts.form.description.edit')
              : t('contacts.form.description.add')}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="cf-phone" className="text-muted-foreground">
              {t('contacts.form.labels.phone')} <span className="text-red-400">*</span>
            </Label>
            <Input
              id="cf-phone"
              value={phone}
            ...
            {dupMatch ? (
              ...
              >
                ...
                <div className="space-y-1">
                  <p>
                    {dupMatch.exact
                      ? t('contacts.form.duplicate.exact')
                      : t('contacts.form.duplicate.similar')}
                  </p>
                  {onViewExisting && (
                    <button
                      type="button"
                    ...
                    >
                      {t('contacts.form.duplicate.viewContact', {
          ...
          <div className="space-y-2">
            <Label htmlFor="cf-company" className="text-muted-foreground">
              {t('contacts.form.labels.company')}
            </Label>
            <Input
              id="cf-company"
              value={company}
          ...
          <div className="space-y-2">
            <Label className="text-muted-foreground">{t('contacts.form.labels.tags')}</Label>
            {loadingTags ? (
              <div className="flex items-center gap-2 text-muted-foreground text-sm">
                <Loader2 className="size-3 animate-spin" />
                {t('contacts.form.tags.loading')}
              </div>
            ) : tags.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                {t('contacts.form.tags.empty')}
              </p>
            ) : (
          ...
          <DialogFooter className="bg-popover border-border">
            ...
            >
              {t('contacts.form.actions.cancel')}
            </Button>
            <Button
              type="submit"
            ...
            >
              ...
              {isEdit ? t('contacts.form.actions.update') : t('contacts.form.actions.create')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
