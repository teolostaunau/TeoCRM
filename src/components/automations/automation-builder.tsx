...
const TRIGGER_LABEL_KEYS: Record<AutomationTriggerType, string> = {
  ...
  first_inbound_message:
    "automations.builder.triggers.firstInboundMessage.label",
  keyword_match:
    "automations.builder.triggers.keywordMatch.label",
  new_contact_created:
    "automations.builder.triggers.newContactCreated.label",
  conversation_assigned:
    "automations.builder.triggers.conversationAssigned.label",
  tag_added:
    "automations.builder.triggers.tagAdded.label",
  time_based:
    "automations.builder.triggers.timeBased.label",
}

const TRIGGER_HINT_KEYS: Record<AutomationTriggerType, string> = {
  new_message_received:
    "automations.builder.triggers.newMessageReceived.hint",
  first_inbound_message:
    "automations.builder.triggers.firstInboundMessage.hint",
  keyword_match:
    "automations.builder.triggers.keywordMatch.hint",
  new_contact_created:
    "automations.builder.triggers.newContactCreated.hint",
  conversation_assigned:
    "automations.builder.triggers.conversationAssigned.hint",
  tag_added:
    "automations.builder.triggers.tagAdded.hint",
  time_based:
    "automations.builder.triggers.timeBased.hint",
}
...
function TagSelect({
  ...
  if (tags.length === 0) {
    return (
      <Input
        placeholder={t("automations.builder.placeholders.tagId")}
        value={value}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <option value="">
          {t("automations.builder.placeholders.selectTag")}
        </option>
        {tags.map((t) => (
...
export function AutomationBuilder({ initial }: { initial: BuilderInitial }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (firstIssue?.message) {
          toast.error(firstIssue.message, {
            ...
            ? t("automations.builder.messages.atPath", {
          ...
          toast.error(
            body?.error ?? t("automations.builder.messages.saveFailed"),
          )
        }
        return
      }
      toast.success(
        isEditing
          ? t("automations.builder.messages.automationSaved")
          : t("automations.builder.messages.automationCreated"),
      )
    ...
    <div className="fixed inset-0 flex flex-col bg-background">
      ...
      <header className="flex flex-shrink-0 items-center gap-2 border-b border-border bg-card/80 px-3 py-3 sm:gap-3 sm:px-4">
        <button
          ...
          aria-label={t("automations.builder.general.backToAutomations")}
        >
        ...
        <input
          ...
          placeholder={t("automations.builder.general.untitledAutomation")}
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span className="hidden sm:inline">
            {t("automations.builder.general.active")}
          </span>
          <Switch
            checked={state.is_active}
            onCheckedChange={(v) => patchTop("is_active", !!v)}
            aria-label={t("automations.builder.general.active")}
          />
        </div>
        <Button
          onClick={save}
        ...
        >
          ...
          {isEditing
            ? t("automations.builder.general.save")
            : t("automations.builder.general.saveDraft")}
        </Button>
      </header>
      ...
      <div className="rounded-lg border border-border border-l-4 border-l-blue-500 bg-card shadow-lg">
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[11px] uppercase tracking-wide text-blue-300">{t("automations.builder.general.trigger")}</div>
    ...
    <div className="space-y-2">
      <div>
        ...
        <Input
          ...
          placeholder={t("automations.builder.placeholders.keywords")}
          className="bg-muted text-foreground"
        />
      </div>
      <div>
        ...
        >
          <option value="contains">{t("automations.builder.conditions.contains")}</option>
          <option value="exact">{t("automations.builder.conditions.exact")}</option>
        </select>
      </div>
    </div>
  )
              ...
              <div className="mt-3 flex items-center justify-between gap-2 border-t border-border pt-3">
                <div className="flex gap-1">
                  <Button
                    ...
                    size="icon"
                    disabled={index === 0}
                    aria-label={t("automations.builder.actions.moveUp")}
                  ...
                  <Button
                    ...
                    aria-label={t("automations.builder.actions.moveDown")}
                    onClick={() => props.moveStepAt(path, 1)}
                  >
                ...
                >
                  ...
                  {t("automations.builder.actions.delete")}
                </Button>
              </div>
            </div>
          )}
...
function ConditionBranches({
  ...
  return (
    ...
    <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
      <BranchColumn label={t("automations.builder.branches.yes")} color="text-primary">
        <StepList {...props} steps={yes} parentPath={yesPath} />
      </BranchColumn>
      <BranchColumn label={t("automations.builder.branches.no")} color="text-rose-400">
    ...
    <div className="relative flex flex-col items-center">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          ...
          aria-label={t("automations.builder.accessibility.addStep")}
        >
      ...
      return (
        <FieldBlock label={t("automations.builder.fields.messageText")}>
          <Textarea
            ...
            placeholder={t("automations.builder.placeholders.messageText")}
        ...
        <>
          ...
          <FieldBlock label={t("automations.builder.fields.value")}>
            <Input
              ...
              placeholder={t("automations.builder.placeholders.contactFieldValue")}
              className="bg-muted text-foreground"
            />
          </FieldBlock>
        </>
        ...
        <>
          <FieldBlock label={t("automations.builder.fields.subject")}>
            <select
              ...
              className="w-full rounded-md border border-border bg-muted px-2 py-1.5 text-sm text-foreground"
            >
              <option value="tag_presence">{t("automations.builder.conditions.tagPresence")}</option>
              <option value="contact_field">{t("automations.builder.conditions.contactField")}</option>
              <option value="message_content">{t("automations.builder.conditions.messageContent")}</option>
              <option value="time_of_day">{t("automations.builder.conditions.timeOfDay")}</option>
            </select>
          </FieldBlock>
            ...
            <Input
              placeholder={
                cfg.subject === "time_of_day"
                  ? t("automations.builder.placeholders.timeRange")
