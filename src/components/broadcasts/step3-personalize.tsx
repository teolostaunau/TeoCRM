...
const contactFields = (t: (key: string) => string) => [
  { value: 'name', label: t('broadcasts.steps.personalize.contactFields.name') },
  { value: 'phone', label: t('broadcasts.steps.personalize.contactFields.phone') },
  { value: 'email', label: t('broadcasts.steps.personalize.contactFields.email') },
  { value: 'company', label: t('broadcasts.steps.personalize.contactFields.company') },
];
...
export function Step3Personalize({
  ...
  const contactFieldOptions = contactFields(t);
  ...
  const previewLabel = firstContact
    ...
    : t('broadcasts.steps.personalize.preview.sampleData');

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-foreground">{t('broadcasts.steps.personalize.title')}</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          {t('broadcasts.steps.personalize.description')}
        </p>
      </div>
      ...
      {mediaHeaderType && (
        <div className="rounded-xl border border-border bg-card/50 p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-foreground">{t('broadcasts.steps.personalize.headerMedia.title')}</p>
          ...
          <label className="mb-1.5 block text-xs font-medium text-muted-foreground">
            {t('broadcasts.steps.personalize.headerMedia.label')}
          </label>
          <Input
            type="url"
            value={headerMediaUrl}
            onChange={(e) => onHeaderMediaUrlChange(e.target.value)}
            placeholder={t('broadcasts.steps.personalize.headerMedia.placeholder')}
            className="border-border bg-muted text-foreground placeholder:text-muted-foreground"
          />
          <p className="mt-1.5 text-xs text-muted-foreground">
            {t('broadcasts.steps.personalize.headerMedia.description', { type: mediaHeaderType })}
          </p>
          ...
          {headerMediaError && (
            <p className="mt-1.5 text-xs text-amber-300">
              {headerMediaError === 'missing'
                ? t('broadcasts.steps.personalize.headerMedia.validation.missing')
                : t('broadcasts.steps.personalize.headerMedia.validation.invalid')}
            </p>
          )}
        </div>
      )}
      ...
      {placeholders.length === 0 && !mediaHeaderType ? (
        <div className="rounded-xl border border-border bg-card/50 p-6 text-center">
          <p className="text-sm text-muted-foreground">
            {t('broadcasts.steps.personalize.emptyState')}
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <label className="mb-1.5 block text-xs font-medium text-muted-foreground">
                      {t('broadcasts.steps.personalize.mapping.type')}
                    </label>
                    <Select
                      value={mapping.type}
                      onValueChange={(val) =>
                    ...
                    >
                      ...
                      <SelectContent className="border-border bg-popover">
                        <SelectItem value="static">{t('broadcasts.steps.personalize.mapping.staticValue')}</SelectItem>
                        <SelectItem value="field">{t('broadcasts.steps.personalize.mapping.contactField')}</SelectItem>
                        <SelectItem value="custom_field">
                          {t('broadcasts.steps.personalize.mapping.customField')}
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-medium text-muted-foreground">
                      {mapping.type === 'static' ? t('broadcasts.steps.personalize.mapping.value') : t('broadcasts.steps.personalize.mapping.field')}
                    </label>
                    {mapping.type === 'static' ? (
                      <Input
                        value={mapping.value}
                        ...
                        placeholder={t('broadcasts.steps.personalize.mapping.placeholder')}
                        className="border-border bg-muted text-foreground placeholder:text-muted-foreground"
                      />
                    ) : mapping.type === 'field' ? (
                      <Select
                      ...
                      >
                        <SelectTrigger className="w-full border-border bg-muted text-foreground">
                          <SelectValue placeholder={t('broadcasts.steps.personalize.mapping.selectField')} />
                        </SelectTrigger>
                        <SelectContent className="border-border bg-popover">
                      ...
                      >
                        <SelectTrigger className="w-full border-border bg-muted text-foreground">
                          <SelectValue
                            placeholder={
                              loadingFields
                                ? t('broadcasts.steps.personalize.mapping.loading')
                                : customFields.length === 0
                                  ? t('broadcasts.steps.personalize.mapping.noCustomFields')
                                  : t('broadcasts.steps.personalize.mapping.selectCustomField')
                            }
                          />
                        </SelectTrigger>
                        <SelectContent className="border-border bg-popover">
      ...
      <div className="rounded-xl border border-border bg-card/50 p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <p className="text-sm font-medium text-foreground">{t('broadcasts.steps.personalize.preview.title')}</p>
      ...
      {unmappedKeys.length > 0 && (
        <div className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
          {t('broadcasts.steps.personalize.validation.missing', { placeholders: unmappedKeys.join(', ') })}
        </div>
      )}
      ...
      <div className="flex items-center justify-between border-t border-border pt-4">
        ...
        >
          ...
          {t('broadcasts.steps.personalize.actions.back')}
        </Button>
        <Button
          onClick={onNext}
        ...
        >
          {t('broadcasts.steps.personalize.actions.next')}
