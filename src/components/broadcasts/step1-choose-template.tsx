...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  useEffect(() => {
    async function fetchTemplates() {
      try {
        ...
        setError(err instanceof Error ? err.message : t('broadcasts.steps.chooseTemplate.errors.load'));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-foreground">{t('broadcasts.steps.chooseTemplate.title')}</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          {t('broadcasts.steps.chooseTemplate.description')}
        </p>
      </div>
      ...
      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-border bg-card/50">
          ...
          <p className="text-sm text-muted-foreground">{t('broadcasts.steps.chooseTemplate.emptyTitle')}</p>
          <p className="mt-1 text-xs text-muted-foreground">{t('broadcasts.steps.chooseTemplate.emptyDescription')}</p>
        </div>
      ) : (
      ...
      <div className="flex items-center justify-between border-t border-border pt-4">
        <Button variant="outline" onClick={onBack} className="border-border text-muted-foreground">
          {t('broadcasts.steps.chooseTemplate.actions.back')}
        </Button>
        <Button
          onClick={onNext}
        ...
        >
          {t('broadcasts.steps.chooseTemplate.actions.next')}
